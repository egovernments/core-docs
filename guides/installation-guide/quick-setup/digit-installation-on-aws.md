...
1.  Generate ssh key pairs (Use either method (a) or method (b)).\
    a. Using online website (not recommended in prod setup. To be only used for demo setups):\
    &#x20; [https://8gwifi.org/sshfunctions.jsp](https://8gwifi.org/sshfunctions.jsp)
...
3.  Open input.yaml file in vscode. You can use the below code to directly open it in VS code:
    ...
    _(In case you are not using vscode, you can open it any editor of your choice)_
4.  Open egov-demo-secret.yaml and add DB password (line number 5), flywayPassword (line number 7) and private key. You can use the following command to open it in VS code:

    `code config-as-code/environments/egov-demo-secrets.yaml`

    Please keep the DB password and flywayPassword same. Private key has to be added inside git-sync key against ssh key (line number 37).
5.  Next go to _infra-as-code/terraform/sample-aws_ and run _init.go_ script to enrich different files based on _input.yaml_. You can run the script using the following command:

    `cd infra-as-code/terraform/sample-aws go run ../scripts/init.go`
6.  Now go to _remote-state_ folder and run terraform using the following commands. It will create a S3 bucket and DynamoDB to maintain terraform state.

    `cd remote-state`\
    `terraform init` \
    `terraform plan` \
    `terraform apply`
7.  Next cd back to sample-aws folder and run terraform to provision infra for DIGIT. Use the following command:
    ...
    _(Add the same DB password which you have added in egov-demo-secret.yaml when prompted after running terraform apply)_
8.  Execute the following command to generate a kubeConfig file and update the volumeIds, DB URL, and other relevant details in the egov-demo.yaml file.

    `terraform output -json | go run ../scripts/envYAMLUpdater.go`
9. Run the export KUBECONFIG command shown on terminal. _(Note: The exact command to run will be printed on terminal. It will be something like this: export KUBECONFIG=\<LOCAL\_KUBECONFIGPATH> )_
10. Next step is deployment of services. Run the digit-installer.go script to install UPYOG using the following command:

    `cd ../../../deploy-as-code/deployer go run digit_installer.go`
11. Once this is done you will get the CNAME of the _nginx-ingress-controller_ using the following command:
