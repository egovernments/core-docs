...
MDMS v2 provides APIs for defining schemas, searching schemas, and adding master data against these defined schemas. All data is now stored in PostgreSQL tables instead of GitHub. MDMS v2 currently also includes the v1 search API for fetching data from the database in the same format as the MDMS v1 search API to ensure backwards compatibility.
...
Additionally, we have two keys which are not part of the standard JSON schema attributes -

* x-unique: specifies the fields in the schema utilising which a unique identifier for each master data will be created.
...
2. Search schema: MDMS v2 has an API to search the schema based on the tenantid, schema code, and unique identifier.
...
4. Search data: MDMS v2 exposes two search APIs - v1 and v2 search, where the v1 search API is completely backwards compatible.
...
1. _/mdms-v2/schema/v1/\_create_ - Takes RequestInfo and SchemaDefinition in the request body. SchemaDefinition has all the attributes which define the schema.
...
3. _/mdms-v2/v2/\_create/{schemaCode}_ - Takes RequestInfo and Mdms in the request body, where the MDMS object has all the information for the master being created, and it takes schemaCode as a path parameter to identify the schema against which data is being created.
...
5. _/mdms-v2/v2/\_update/{schemaCode} -_ Takes RequestInfo and Mdms in the request body, where the MDMS object has all the information for the master being updated, and it takes schemaCode as a path param to identify the schema against which data is being updated.
...
[https://api.postman.com/collections/12892142-771842a6-fe9d-4ba7-93ba-2fffdb525055?access\_key=PMAT-01H5PA8YTK7H1357VGDQ4YMW7W](https://api.postman.com/collections/12892142-771842a6-fe9d-4ba7-93ba-2fffdb525055?access_key=PMAT-01H5PA8YTK7H1357VGDQ4YMW7W)
